                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            $sql="UPDATE users SET username=?, serialnumber=?, gender=?, email=?, user_date=CURDATE(), device_uid=?, device_dep=?, add_card=1 ,password_user=? WHERE id=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                ...
                                mysqli_stmt_bind_param($result, "sdssssis", $Uname, $Number, $Gender, $Email, $dev_uid, $dev_name, $Password_user,$user_id );
                                mysqli_stmt_execute($result);
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql="UPDATE users SET username=?, serialnumber=?, gender=?, email=?, device_uid=?, device_dep=? ,password_user=? WHERE id=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
